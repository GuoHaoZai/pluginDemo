...
public class GenerateFieldGeneralAction extends PsiElementBaseIntentionAction {
    ...
    public void invoke(@NotNull Project project, Editor editor, @NotNull PsiElement element) throws IncorrectOperationException {
        ...
        Document document = PsiDocumentUtils.getDocument(psiClass);
        for (PsiField psiField : psiFields) {
            TextRange textRange = psiField.getTextRange();
            String formatString = PsiToolUtils.calculateFormatString(psiField);
            document.insertString(textRange.getStartOffset(), fieldString + formatString);
            PsiDocumentUtils.commitAndSaveDocument(psiClass, document);
        }
    }
    ...
    public boolean isAvailable(@NotNull Project project, Editor editor, @NotNull PsiElement element) {
        ...
        return Objects.nonNull(parentOfType);
    }
